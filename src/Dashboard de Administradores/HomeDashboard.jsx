...
const HomeDashboard = () => {
  ...
  const handleDeleteCategory = async (categoryName) => {
    const confirmDelete = window.confirm(`¿Está seguro que desea eliminar la categoría "${categoryName}"?`);
    ...
    <div className="flex flex-col md:flex-row bg-gradient-to-r from-blue-500 via-blue-400 to-green-500 text-white min-h-screen">
      <div className="flex-1 p-8">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to='/dashboard' className="text-sm bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded text-center">
            Admin users
          </Link>
        </div>
        <div className="flex justify-between max-w-md mx-auto mb-4">
          <div className="w-1/2 mr-2">
            <Link to='/dashboard/creationProduct' className="block py-2 px-4 bg-blue-500 hover:bg-blue-700 text-white font-bold rounded text-center">
              Creación de Producto
            </Link>
          </div>
          <div className="w-1/2 ml-2">
            <Link to='/dashboard/creationCategory' className="block py-2 px-4 bg-blue-500 hover:bg-blue-700 text-white font-bold rounded text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {productData.map((product) => (
              <div key={product.id} className="bg-white p-4 rounded shadow text-black mb-4">
                <h3 className="text-lg font-bold mb-2 text-center text-gray-900">{product.title}</h3>

                <img src={product.image} alt={product.title} className="mx-auto mb-4 max-w-full" style={{ height: '200px' }} />
                ...
                <div className="flex justify-center">
                  ...
                  <Link to={`/dashboard/modifications/product/${product.id}`} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Editar
                  </Link>
                </div>
              </div>
