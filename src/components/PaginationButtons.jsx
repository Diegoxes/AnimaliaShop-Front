import React, { useEffect } from "react"; //sacar useeffect
...
const PaginationButtons = () => {
  ...
  return (
    <nav aria-label="Page Navigation" className="mx-auto my-10 flex max-w-xs justify-between space-x-2 rounded-md bg-white py-2">
      <button className="flex items-center space-x-1 font-medium hover:text-blue-600" aria-label="First Page" onClick={() => handlePageClick(1)}>
      ...
      <button className="flex items-center space-x-1 font-medium hover:text-blue-600" aria-label="Previous Page" onClick={() => dispatch(changePage('prev'))}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
        ...
        {generatePageNumbers().map((pageNumber) => (
          <li key={pageNumber}>
            <button
              ...
              currentPage === pageNumber ? "text-blue-600" : "text-gray-400"
      ...
      <button className="flex items-center space-x-1 font-medium hover:text-blue-600" aria-label="Next Page" onClick={() => dispatch(changePage('next'))}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
          <path fillRule="evenodd" d="M16.28 11.47a.75.75 0 010 1.06l-7.5 7.5a.75.75 0 01-1.06-1.06L14.69 12 7.72 5.03a.75.75 0 011.06-1.06l7.5 7.5z" clipRule="evenodd" />
        </svg>
      </button>
      <button className="flex items-center space-x-1 font-medium hover:text-blue-600" aria-label="Last Page" onClick={() => handlePageClick(totalPages - 1)}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
